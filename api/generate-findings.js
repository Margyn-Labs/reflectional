...
/* ============================================================================
   ...
   The actual Claude-calling logic lives in api/_lib/importMapper.js — it's
   shared with the WhatsApp media path in api/whatsapp.js (2026-09-12),
   which runs the identical mapper but parks the result in
   import_suggestions instead of returning it synchronously, since there's
   no live browser tab to show a review screen to on that path.
   ============================================================================ */
// importMapper.js is CommonJS (module.exports) to match api/whatsapp.js's
// side of this shared file — import it via the default-export interop
// rather than named imports, which is the reliable way to pull a CJS
// module's exports into an ESM file.
import importMapperPkg from './_lib/importMapper.js';
const { runImportMapper, sanitizeProposal, normImportDate } = importMapperPkg;
export { sanitizeProposal, normImportDate }; // re-exported for test-fixtures/test-sanitizer.mjs
...
async function handleParseImport(req, res, body) {
  ...
  try {
    ...
    const { proposal } = await runImportMapper(body, apiKey);
